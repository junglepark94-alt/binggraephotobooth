...
프레임을 골라 4컷을 촬영하고, 프레임에 합성해 저장·공유하거나 광장 게시판에 자랑할 수 있다.
...
main(메인) → letter(편지) → map(축제 지도) → select(프레임 선택)
                                   │
                                   ├─ shoot(촬영 ×4컷) → result(합성·꾸미기·저장·공유·게시판 업로드)
                                   ├─ board(광장 게시판)   … 다른 사람 4컷 구경 / 좋아요
                                   ├─ draw(뽑기)          … 스크래치 운세
                                   └─ end(축제 종료)
```
...
- `src/screens/`
  ...
  - **ResultScreen** — 4컷을 프레임에 합성 → `PhotoEditor`로 스티커/그리기 꾸미기 →
    PNG 저장 / Web Share 공유 / 게시판 업로드. 스티커·획은 이미지 대비 **비율(0~1)** 로 저장해
    화면 표시와 원본 해상도 내보내기가 일치. 저장·공유는 `exportPng()`로 원본 해상도 재합성
  - **FestivalMap** — 축제 지도 + 핫스팟 미니게임(인벤토리 진행바)
  - **PlazaBoard** — 광장 게시판(최신/좋아요순, 60장씩 무한 스크롤)
  - **DrawScreen** — 스크래치 운세
...
- `src/lib/plaza.ts` — 광장 게시판 서버 함수(RPC): 등록/목록/좋아요 + 어드민 삭제, IP 레이트리밋
...
- 결과 화면의 **"주민들에게 자랑하기"** 로만 서버 전송 (축소 JPEG data URL).
- 저장소는 `REDIS_URL` 유무로 자동 선택: 있으면 **Bun 내장 Redis**(영구·공유), 없으면 인메모리(재시작 시 초기화).
- 최신 **200장 롤링 보관**, 장당 이미지 상한 ~650KB, IP별 슬라이딩 윈도우 레이트리밋.
- 어드민(`/admin`)은 **`ADMIN_PASSWORD` 환경변수 필수** — 미설정 시 비활성화(소스에 기본 비밀번호 없음).
...
- 게시판을 영구 보관하려면 Railway에 Redis를 붙이고 `REDIS_URL` 설정.
