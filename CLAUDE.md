...
- `src/assets/` — 프레임 4종(binggraeus/melonaprince/bravocone/bananamilk), 배경·버튼·아이콘 에셋.
  ...
  (픽셀이 원본 PNG와 동일 → 흰배경 키잉/크롭 동작 보존), 순수 표시용 배경·로고는 **손실**(q82~88).
  프레임 4종은 **손실 q92** — 초록 슬롯 감지·초록→투명·바깥 흰배경 제거 결과가 무손실과 동일함을
  검증하고 적용했다(슬롯 4/4 일치·코너≤1px, 투명화 차이는 1px 경계선뿐).
...
### 프레임을 추가/수정하려면
1. 프레임을 만든 뒤 **WebP**로 변환해 `src/assets/`에 넣는다 — 사진이 들어갈 자리는 **순수 초록색**으로 채운다 (감지 기준: `g>180, r<120, b<120, g>r+80, g>b+80`, `photobooth.ts`의 `isPlaceholderGreen`). 초록 감지가 픽셀에 의존하므로 손실 압축을 쓸 거면 무손실 대비 슬롯 4개가 그대로 잡히는지 반드시 검증할 것(기존 4종은 q92로 검증 완료). 확신이 없으면 무손실로.
